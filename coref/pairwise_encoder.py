...
class PairwiseEncoder(torch.nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.genre2int = {g: gi for gi, g in enumerate(["bc", "bn", "mz", "nw",
                                                        "pt", "tc", "wb"])}
        self.genre_emb = torch.nn.Embedding(len(self.genre2int), self.emb_size)
        ...
        # self.speaker_emb = torch.nn.Embedding(20, emb_size)

        self.dropout = torch.nn.Dropout(config.dropout_rate)
        self.shape = self.emb_size * 2  # genre, speaker\
    ...
    def forward(self,  # type: ignore  # pylint: disable=arguments-differ  #35566 in pytorch
        ...
        genre = torch.tensor(self.genre2int[doc["document_id"][:2]],
                             device=self.device).repeat(speaker_map.shape[0])
        genre = self.genre_emb(genre)

        return self.dropout(torch.cat((speaker_map, genre), dim=-1))

    def _speaker_map(self, doc: Doc) -> List[int]:
        """
        ...
        onehot_speaker = torch.eye(self.emb_size, device=self.device)

        # word id -> speaker id
        return torch.cat([onehot_speaker[str2int[s]].unsqueeze(0) for s in doc["speaker"]], 0)
